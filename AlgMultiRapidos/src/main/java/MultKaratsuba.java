...
public class MultKaratsuba {
    public static void main(String[] args) {
        Scanner in = new Scanner(System.in);
        testExamples();

        System.out.println("\n---Faça a sua conta de multiplicação Karatsuba---");
    ...
    private static int mult(int a, int b, int n, int base) {
        if(n > 1){
            ...
            int A1B1 = mult(A0, B0, n/2, base);
            int A0B0 = mult(A1, B0, n/2, base);
            int M3 = mult(A1 + A0, B1+B0, n/2, base);

            return (int) Math.pow(base, n) * A1B1 + (int) Math.pow(base, n/2) * (M3 - A1B1 - A0B0) + A0B0;
        }
        else{
            return a * b;
        }      
